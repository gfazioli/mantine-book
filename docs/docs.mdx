...
And the **closed book is compact**: at either end the single visible page
is centered on the play-zone, sliding into the two-page spread as the
cover opens — the slide is a CSS transition, disabled under
prefers-reduced-motion. Drag the cover slowly to feel the difference: it
stays flat all the way over.

<Demo data={demos.withCover} />
...
entirely. So the 100-page book below costs the same GPU resources as a
single sheet — each page's hue advances by the golden angle, so
neighbouring pages contrast strongly while the whole book still spans the
color wheel. Drag the slider for a long riffle:

<Demo data={demos.largeBook} />
